...
const DEFAULT_OPTIONS = [
  { color: '#FF3B3B', name: 'Red', weight: 1, isWinning: false },
  { color: '#00C9A7', name: 'Mint', weight: 1, isWinning: false },
  { color: '#4D7FFF', name: 'Blue', weight: 1, isWinning: false },
  { color: '#00B894', name: 'Green', weight: 1, isWinning: false },
  { color: '#9B51E0', name: 'Purple', weight: 1, isWinning: false },
  { color: '#FF66B2', name: 'Pink', weight: 1, isWinning: false },
  { color: '#FFD700', name: 'Yellow', weight: 1, isWinning: false },
  { color: '#FF9933', name: 'Orange', weight: 1, isWinning: false }
];
...
const RouletteWheel = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const probabilityIncrease = Math.floor(sentiment);
      
      setNotification({
        isVisible: true,
        message: `メッセージを受け付けました！ 当たり確率が${probabilityIncrease}%上昇します✨`
      });
      ...
      const updatedOptions = rouletteState.options.map(option => ({
        ...option,
        isWinning: Math.random() < (sentiment / 100)
      }));
      ...
      <div className="w-full max-w-[min(90vw,32rem)] bg-slate-900/50 text-white p-4 sm:p-6 md:p-8 rounded-xl shadow-2xl backdrop-blur-sm">
        {notification.isVisible && (
          <div className="fixed top-20 left-1/2 transform -translate-x-1/2 z-50 animate-fade-in">
            <div className="bg-gradient-to-r from-blue-500 to-purple-500 text-white px-6 py-3 rounded-full shadow-lg backdrop-blur-sm">
          ...
          <div className="text-center space-y-4 sm:space-y-6">
            ...
            {rouletteState.result && !rouletteState.isSpinning && (
              <div className="flex flex-col items-center space-y-2">
                ...
                <div 
                  className={`text-xl sm:text-2xl font-bold px-4 sm:px-6 py-1.5 sm:py-2 rounded-lg ${
                    rouletteState.result.isWinning 
                      ? 'bg-gradient-to-r from-yellow-400 to-yellow-600 animate-pulse' 
                      : 'bg-white/10'
                  }`}
                  style={{ 
                    color: rouletteState.result.isWinning ? 'white' : rouletteState.result.color,
                    textShadow: `0 0 10px ${rouletteState.result.color}40`
                  }}
                >
                  <span>{rouletteState.result.name}</span>
                  {rouletteState.result.isWinning && (
                    <span className="ml-2 text-yellow-300">★当たり★</span>
                  )}
                </div>
              </div>
            )}
